...
class TaskRewardService:
    ...
    def create_project(
        ...
        project = TaskProject(name=name, status=status, sort_order=sort_order)
    ...
    def update_project(self, project_id: int, **changes: object) -> TaskProject:
    ...
    def update_task_template(self, template_id: int, **changes: object) -> TaskTemplate:
    ...
    def list_templates(self, project_id: Optional[int] = None) -> list[TaskTemplate]:
    ...
    def create_daily_task(
        ...
        template = self._get_template(task_template_id)
    ...
    def list_daily_tasks(self, date: datetime.date) -> list[DailyTask]:
        return self.session.scalars(
            select(DailyTask)
    ...
    def complete_daily_task(
        ...
        task = self._get_daily_task(task_id)
    ...
    def reopen_daily_task(self, task_id: int) -> DailyTask:
        task = self._get_daily_task(task_id)
        if task.status != "completed":
            return task
    ...
    def get_reward_summary(self, date: Optional[datetime.date] = None) -> RewardSummaryRead:
        target_date = date or datetime.date.today()
        current_balance = self.session.scalar(
    ...
    def list_reward_ledger(self, limit: int = 20) -> list[RewardLedger]:
        return self.session.scalars(
            select(RewardLedger)
    ...
    def spend_reward(self, amount: int, reason: str) -> RewardLedger:
        current_balance = self.session.scalar(
            select(func.coalesce(func.sum(RewardLedger.amount), 0))
        )
    ...
    def _get_template(self, template_id: int) -> TaskTemplate:
        template = self.session.get(TaskTemplate, template_id)
        if template is None:
            raise ValueError("任务模板不存在")
        return template

    def _get_project(self, project_id: int) -> TaskProject:
        project = self.session.get(TaskProject, project_id)
        if project is None:
            raise ValueError("项目不存在")
        return project

    def _get_daily_task(self, task_id: int) -> DailyTask:
